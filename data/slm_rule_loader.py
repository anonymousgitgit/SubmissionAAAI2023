        ...
        if dgl_format:
            if num_node_types == 1:
                if task == 'classification':
                    ...
                    _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, stmt_ids, extra_src_info, src_token_ids, rule_ids, all_rules, rule_token_ids = list(zip(*batch))
                    ...
                    rule_length = list(map(len, rule_ids))
                    rule_ids = _pad_batch_2D(rule_ids, value = PAD_IDX)
                    rule_token_ids = _pad_batch_3D(rule_token_ids, value = PAD_IDX)
            ...
            if num_node_types == 1:
                if task == 'classification':
                    ...
                    return {'num_nodes': num_nodes, 'last_stmts': last_stmt_indices}, ast_node_index, batch_tensor_buckets, dgl.batch(graphs), in_degrees, stmt_ids, extra_src_info, torch.tensor(rule_ids), all_rules, torch.tensor(rule_length), torch.tensor(rule_token_ids)
            elif num_node_types == 2:
...
def eval_collate(vocab_obj, dgl_format, num_node_types, task):
    def fn(batch):
        if dgl_format:
            ...
            if num_node_types == 1:
                if task == 'classification':
                    return {'num_nodes': num_nodes, 'last_stmts': last_stmt_indices}, ast_node_index, batch_tensor_tree, dgl.batch(graphs), in_degrees, torch.tensor(labels)
                elif task == 'summarization':
