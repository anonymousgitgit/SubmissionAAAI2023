...
class PCNDatasetJava(Dataset):
    def __init__(self, tokenizer, classmap, vocab_obj, lang_obj, paths, parser, dgl_format, num_node_types):
    ...
    def __getitem__(self, index):
        ...
        text = open(path).read()
        label = self.classmap[path.split(os.ts.sep)[-2]]
    ...
    def convert_raw_ast_into_full_format(self, root):
        ...
        while queue:
            ...
            if current_node['node_type'] == 'string_literal':
                #TODO: 
                sub_tokens = self.tokenizer.encode("___str___").tokens
            else:
