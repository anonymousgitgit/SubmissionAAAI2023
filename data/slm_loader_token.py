...
def make_batch(batch_data):
    ...
    batch_size = []
    tree_index = []
    batch_token_node_ids = []
    batch_token_node_lens = []
    for tree_data in batch_data:
        ...
        batch_token_node_ids.append(tree_data['token_node_ids'])
        batch_token_node_lens.append(tree_data['token_code_lens'])
    ...
    # batch_token_node_lens = _pad_batch_2D(batch_token_node_lens)
    batch_token_node_ids = np.array(_pad_batch_2D(batch_token_node_ids, value = -1)) + 1
    # [[[]]]
    ...
    batch_obj = {
        ...
        'batch_token_node_ids': batch_token_node_ids,
        'batch_token_node_lens': batch_token_node_lens,
...
def get_ast_index(vocab_obj, ast_nodes, num_nodes, tokenizer):
    data = {}
    offset = 0
    if tokenizer is None:
        fn = split_identifier_into_parts
    else:
        fn = lambda x: tokenizer.encode(x).tokens
    for ast, num_node in zip(ast_nodes, num_nodes):
        for node, info in ast.items():
            tokens = fn(info['node_token'])
            data[node + offset] = {
                'node_type_index': torch.tensor(vocab_obj.get_id_from_node_type(info['node_type'])),
                'node_sub_tokens': tokens, 
                'node_sub_token_ids': torch.tensor(list(map(vocab_obj.get_id_from_sub_token, tokens)))
            }
        offset += num_node
    return data
...
def train_collate(vocab_obj, dgl_format, num_node_types, task, tokenizer):
    def fn(batch):
        if dgl_format:
            if num_node_types == 1:
                if task == 'classification':
                    _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, labels = list(zip(*batch))
                elif task == 'summarization':
                    _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, stmt_ids, extra_src_info, src_token_ids, tgt_token_ids, tgt_tokens, tgt_texts = list(zip(*batch))
                    
                    src_vocabs = []
                    extra_src_token_ids = []
                    src_token_ids_vocab = []
                    src_lengths = []
                    for item in extra_src_info:
                        src_vocabs.append(item['src_vocab'])
                        extra_src_token_ids.append(item['extra_src_token_ids'])
                        src_token_ids_vocab.append(item['src_token_ids_vocab'])
                        src_lengths.append(len(item['src_token_ids_vocab']))
                    src_token_ids_vocab = torch.LongTensor(_pad_batch_2D(src_token_ids_vocab))
                    src_lengths = torch.Tensor(src_lengths)
                    src_map = make_src_map(extra_src_token_ids)
                    
                    alignments = []
                    
                    for i, (src_vocab, _tgt_tokens) in enumerate(zip(src_vocabs, tgt_tokens)):
                        align_mask = torch.LongTensor([src_vocab[w] for w in _tgt_tokens])
                        alignments.append(align_mask)

                    alignments = align(alignments)
                    
                    extra_src_info = {'src_map': src_map, 'src_vocabs': src_vocabs, 'alignments': alignments, 'src_token_ids_vocab': src_token_ids_vocab, 'src_lengths': src_lengths}
                    tgt_token_ids = _pad_batch_2D(tgt_token_ids, value = PAD_IDX)
                num_nodes = [g.num_nodes() for g in graphs]
                last_stmt_indices = adjust_tree_index(_tree_data, num_nodes)
                ast_node_index = get_ast_index(vocab_obj, _ast_nodes, num_nodes, tokenizer)
                ...
                ast_node_index = get_ast_index(vocab_obj, _ast_nodes, num_ast_nodes, tokenizer)
            ...
            if num_node_types == 1:
                if task == 'classification':
                    ...
                    return {'num_nodes': num_nodes, 'last_stmts': last_stmt_indices}, ast_node_index, batch_tensor_buckets, dgl.batch(graphs), in_degrees, stmt_ids, extra_src_info, torch.tensor(tgt_token_ids), tgt_texts
            elif num_node_types == 2:
...
def make_data_loader(dataset, vocab_obj, batch_size, task = 'classification', dgl_format = True, num_node_types = 1, shuffle = False, training = True, num_workers = 0, *, tokenizer = None):
    return DataLoader(dataset, batch_size = batch_size, collate_fn = (train_collate)(vocab_obj, dgl_format, num_node_types, task, tokenizer = tokenizer), shuffle = shuffle, num_workers = num_workers)
