...
class TripletDevignDataset(Dataset):
    def __init__(self, vocab_obj, data, parser, dgl_format, num_node_types):
        self.data = data
        self.parser = parser
        self.vocab_obj = vocab_obj
        self.dgl_format = dgl_format
        self.num_node_types = num_node_types
        self.pos_ids, self.neg_ids = [], []
        self.split_data()
    def __len__(self):
        return len(self.data)
    def __getitem__(self, index):
        item = self.data[index]
        anchor_label = item['target']
        if anchor_label:
            same_index = np.random.choice(self.pos_ids[self.pos_ids != index], 1)[0]
            dis_index = np.random.choice(self.neg_ids, 1)[0]
        else:
            same_index = np.random.choice(self.neg_ids[self.neg_ids != index], 1)[0]
            dis_index = np.random.choice(self.pos_ids, 1)[0]
        anchor_value = self.process_one_sample(item)
        pos_value = self.process_one_sample(self.data[same_index])
        neg_value = self.process_one_sample(self.data[dis_index])
        return anchor_value, pos_value, neg_value
    ...
    def split_data(self):
        for i, item in enumerate(self.data):
            (self.pos_ids if item['target'] == 1 else self.neg_ids).append(i)
        self.pos_ids = np.array(self.pos_ids)
        self.neg_ids = np.array(self.neg_ids) 
...
class TripletDevignSampler(Sampler):
    def __init__(self, pos_ids, neg_ids):
        self.pos_ids = pos_ids
        self.neg_ids = neg_ids
    def __iter__(self):
        ids = np.random.choice(self.neg_ids, size = len(self.pos_ids), replace = False)
        ids = np.hstack((ids, self.pos_ids))
        ids = np.random.permutation(ids)
        return iter(ids.tolist())
    def __len__(self):
        return 2 * len(self.pos_ids)