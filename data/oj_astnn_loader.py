...
def clone_train_collate(vocab_obj, dgl_format, num_node_types, task):
    def fn(batch):
        samples_1, samples_2, labels = list(zip(*batch))
        merge_batch = samples_1 + samples_2
        if dgl_format:
            if num_node_types == 1:
                if task == 'clone':
            ...
            if num_node_types == 1:
                if task == 'clone':
                    return {'num_nodes': num_nodes, 'last_stmts': last_stmt_indices}, ast_node_index, batch_tensor_buckets, dgl.batch(graphs), in_degrees, torch.tensor(labels).float()
    return fn

def make_clone_data_loader(dataset, vocab_obj, batch_size, task = 'clone', dgl_format = True, num_node_types = 1, shuffle = False, training = True, num_workers = 0, sampler = None):
    if sampler is not None:
        return DataLoader(dataset, batch_sampler = sampler, collate_fn = (clone_train_collate)(vocab_obj, dgl_format, num_node_types, task), num_workers = num_workers)
    
    return DataLoader(dataset, batch_size = batch_size, collate_fn = (clone_train_collate)(vocab_obj, dgl_format, num_node_types, task), shuffle = shuffle, num_workers = num_workers)
