...
class MaskedPCNDataset(Dataset):
    def __init__(self, vocab_obj, lang_obj, paths, texts, parser, dgl_format, num_node_types, max_seq_length):
        self.paths = paths
        self.texts = texts
        self.parser = parser
        ...
        from data.mask_convert_cpp import convert_into_cpp_graph
        self.convert_into_cpp_graph = convert_into_cpp_graph
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        # print('path', path)
        refactor_json, ast_nodes, raw_stmts, stmt_indices, G, masked_stmt, src_tokens = self.convert_into_cpp_graph(self.parser.parse(text.encode()), text, self.max_seq_length)
        _tgt_tokens = masked_stmt['tokens']
        tgt_tokens = []
        for token in _tgt_tokens:
            tgt_tokens.extend(split_identifier_into_parts(token))
        ...
        if self.dgl_format:
            if self.num_node_types == 1:
                return_value = ast_nodes, list(map(self.extract_tree, tree_data)), *self.convert_nx_into_dgl(G, self.num_node_types, ast_mapping, stmt_mapping), stmt_indices, extra_src_info, src_token_ids, tgt_token_ids, tgt_tokens, [' '.join(tgt_tokens[1:-1])]
            elif self.num_node_types == 2:
                return_value = self.adjust_ast_node_index(ast_nodes, ast_mapping), self.adjust_tree_index(list(map(self.extract_tree, tree_data)), stmt_mapping), *self.convert_nx_into_dgl(G, self.num_node_types, ast_mapping, stmt_mapping), {'ast_nodes': G.number_of_nodes() - len(stmt_indices), 'stmt_nodes': len(stmt_indices)}, stmt_indices, extra_src_info, src_token_ids, tgt_token_ids, tgt_tokens, [' '.join(tgt_tokens[1:-1])]
        else:
            return_value = self.adjust_ast_node_index(ast_nodes, ast_mapping), self.adjust_tree_index(list(map(self.extract_tree, tree_data)), stmt_mapping), *self.convert_nx_into_pyg(G, stmt_indices, ast_mapping, stmt_mapping), {'ast_nodes': G.number_of_nodes() - len(stmt_indices), 'stmt_nodes': len(stmt_indices)}, stmt_indices, src_token_ids, tgt_token_ids, tgt_tokens, [' '.join(tgt_tokens[1:-1])]
        return return_value
...
class MaskedPCNDatasetBpe(Dataset):
    def __init__(self, vocab_obj, lang_obj, paths, texts, parser, dgl_format, num_node_types, max_seq_length, tokenizer):
        self.paths = paths
        self.texts = texts
        self.parser = parser
        self.vocab_obj = vocab_obj
        ...
        from data.mask_convert_cpp_bpe import convert_into_cpp_graph
        self.convert_into_cpp_graph = convert_into_cpp_graph
    def __len__(self):
        return len(self.texts)
    def __getitem__(self, index):
        ...
        path = self.paths[index]
        # print('path', path)
        refactor_json, ast_nodes, raw_stmts, stmt_indices, G, masked_stmt, src_tokens = self.convert_into_cpp_graph(self.parser.parse(text.encode()), text, self.max_seq_length, self.tokenizer)
