...
def triplet_train_collate(vocab_obj, dgl_format, num_node_types, task):
    def fn(batch):
        anchor_samples, pos_samples, neg_samples = list(zip(*batch))
        merge_batch = anchor_samples + pos_samples + neg_samples
        if dgl_format:
            if num_node_types == 1:
                if task == 'classification':
                    _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, labels = list(zip(*merge_batch))
                elif task == 'summarization':
                    _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, stmt_ids, word_ids, word_lst = list(zip(*merge_batch))
                ...
                _ast_nodes, _tree_data, graphs, in_degrees, out_degrees, num_nodes, labels = list(zip(*merge_batch))
...
def make_triplet_data_loader(dataset, vocab_obj, batch_size, task = 'classification', dgl_format = True, num_node_types = 1, shuffle = False, training = True, num_workers = 0, sampler = None):
    if sampler is not None:
        shuffle = False
    return DataLoader(dataset, batch_size = batch_size, collate_fn = (triplet_train_collate)(vocab_obj, dgl_format, num_node_types, task), shuffle = shuffle, num_workers = num_workers, sampler = sampler)
