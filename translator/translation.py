...
class TranslationBuilder(object):
    ...
    def __init__(self, tgt_vocab, tokenizer, n_best=1, replace_unk=False):
    ...
    def _build_target_tokens(self, src_vocab, src_raw, pred, attn):
        ...
        for tok in pred:
            ...
            if tok == constants.SOS_IDX:
                continue
            if tok == constants.EOS_IDX:
                break

            if tok < len(self.tgt_vocab):
                tokens.append(self.tokenizer.token_to_id(self.tgt_vocab.inverse[tok]))
            else:
                tokens.append(self.tokenizer.token_to_id(src_vocab[tok - len(self.tgt_vocab)]))

        if self.replace_unk and (attn is not None):
            for i in range(len(tokens)):
                if tokens[i] not in ['[OOV]', '[UNK]']:
                    _, max_index = attn[i].max(0)
                    tokens[i] = self.tokenizer.token_to_id(src_raw[max_index.item()])
    ...
    def from_batch(self, translation_batch, src_raw, targets, src_vocabs):
        ...
        for b in range(batch_size):
            ...
            _raw =  src_raw[b] if src_raw else None
            pred_sents = [self._build_target_tokens(
                src_vocab, _raw,
                preds[b][n], attn[b][n])
                for n in range(self.n_best)]
            # print('pred', pred_sents)
